...
Checked: 2026-08-31. No archives downloaded. Classes: `dandelion`=0, `clover`=1, `thistle`=2, `mallow`=3 (operator-added; public RGB survey is still thin on mallow). Turf is background.
...
|---|---|---|---:|---:|---:|---|---|---|
| iNaturalist live API (research-grade + photos) | https://api.inaturalist.org/v1/observations — taxa 47602 / 55745+51875 / 60132+52989+76007 | Per-photo CC mix. Majority **CC-BY-NC**. Commercial-safe = **CC0 + CC-BY only**. Also iNat ToS | 164,698 obs (US 60,460). CC0 5,069 + CC-BY 13,647 | White 246,498 (US 115,900); red 232,600 (US 83,846). CC0+CC-BY minority | C. arvense 160,183 (US 46,941); C. vulgare 155,357 (US 57,281); C. nutans 40,760 (US 27,149) | Mixed citizen photos, bloom-heavy, not 1–10 m nadir | No | Obs ≠ images. Filter US + CC0/CC-BY, then box. Do not scrape against ToS. Day-scale drift only vs 2026-08-30 |
...
| Manitoba weed seedlings (Dryad) | https://doi.org/10.5061/dryad.gtht76hhz — paper 10.1371/journal.pone.0243923 | Dryad **CC0-1.0** (confirmed on record) | **4,797** *T. officinale* | none | **4,706** *C. arvense* | Lab robot, pots/soil, not turf. 34,666 RGB classif. | No (image-level) | **6.90 GB**. Good pretrain, large domain gap |
...
| Perrenial Plants Detection (Geisler 2021) | https://www.kaggle.com/datasets/benediktgeisler/perrenial-plants-detection — also https://datasetninja.com/perrenial-plants | **MIT** (stated on Kaggle) | **16 images / 18 boxes** | none | *C. arvense* **70 images / 77 boxes** | Handheld close-up RGB, southern German plant nursery (perennial beds), 4000×6000 — not US lawn | Yes (COCO) | New public species-labeled boxes for Taraxacum + Cirsium. Tiny dandelion count; nursery soil/mulch domain gap; no clover |
| Weed25 (Wang et al. 2022) | Paper DOI 10.3389/fpls.2022.1053329; data on Baidu Netdisk only | Paper CC BY; dataset hosting awkward for US ingest | 0 | 0 | Field thistle *C. arvense* ~565 images | Field close-up RGB (China) | Detection labels expected from paper methods — confirm on unpack | Largest newly surfaced Cirsium pool not previously tracked. Skip for lawn 3-class until hosting/labels verified. Thistle-only. Thin/awkward host |
...
Looked at, not added (brief): LibreYOLO grass-weeds (dock, not our classes); WeedsGalore (maize MSI); WE3DS (*Silybum*); Kura Clover (wrong *Trifolium*); 3W/3W-Pro (*Cirsium*, use restricted); Jiang 2023 (ideal domain, unreleased); Roboflow remixes of the above.
...
- **Crabgrass:** keep in negatives. Yu 2019 false-positive’d *Digitaria* as dandelion. Not a class.
- **Mallow / ground ivy:** operator-added id 3. No public US-lawn detection set yet; backyard inbox is the source.
...
8. Perrenial Plants Detection (MIT, tiny) — low-priority box set after operator decides.
9. Weed25 — only after Baidu/label verify.

Do not mix CottonWeed crabgrass, *Sonchus*, *Erigeron*, *Echinops* globe thistle, or crimson clover into YOLO names. Do not map ground ivy to clover.
