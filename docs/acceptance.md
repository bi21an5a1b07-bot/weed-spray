...
| **Gazebo (opt-in)** | `make sitl-gz`; backend `WEED_HOVER_ALTITUDE_MODE=offboard_agl` `WEED_LIDAR_EXPECTED=1` | Vehicle cam on `rtsp://127.0.0.1:8554/cam`. Hover: lidar-expected → latch scan-height stream (flat ds≈rel ok) → NED → in-band → AGL hold. SIH must not set the flag. Full-green **not yet** — live step 7 UAT is operator WSL on [#19](https://github.com/bi21an5a1b07-bot/weed-spray/issues/19). Do **not** claim exit `0` |
...
WEED_HOVER_ALTITUDE_MODE=offboard_agl WEED_LIDAR_EXPECTED=1 uv run weed-spray
(cd dashboard && npm run dev)
make accept
```

Those env vars belong on the **backend**, not on `make sitl-gz`. `offboard_agl` without `WEED_LIDAR_EXPECTED=1` is refused (SIH has no belly lidar). Latch scan-height stream **before** NED hover: ds and `relative_alt` both `[1, 5]` m (flat ds≈rel **unlocks** — Gazebo). Lag / `relative_alt is None` do not. After descend: trusted AGL in 0.15–0.30 m **and** that latch. Missing a gate → `RuntimeError`, no TERRAIN_ALT, no pulse. Step 7 still grades `hover_agl_m[]` from trusted `DISTANCE_SENSOR` (not `relative_alt` / local `z`). Do not claim exit `0` until that run is green. If terrain-alt Offboard does not hold, paste `var/last-run.md` — do not invent `EKF2_RNG_*` / `MPC_ALT_MODE` workarounds.
...
Exit `0` only with a real rangefinder in the 0.15–0.30 m band (hardware, or Gazebo after live Offboard AGL hover UAT on #19). App seam: `WEED_HOVER_ALTITUDE_MODE=offboard_agl` + `WEED_LIDAR_EXPECTED=1` on the backend (SIH must not set the lidar flag). Opt-in `make sitl-gz` has vehicle cam on `8554/cam` but still does **not** deliver full-green / exit `0`. Default compose remains SIH.
