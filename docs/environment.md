...
| `WEED_HOVER_ALTITUDE_MODE` | `ned` | `ned` (SIH NED down) or `offboard_agl` (NED approach → trusted in-band lidar → MAVSDK AGL hold). SIH mirrors never mark stream alive; no TERRAIN_ALT until in-band |
