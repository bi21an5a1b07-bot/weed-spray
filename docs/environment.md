...
| `WEED_HOVER_ALTITUDE_MODE` | `ned` | `ned` (SIH NED down) or `offboard_agl` (MAVSDK AGL / PX4 terrain-alt Offboard). Pre-goto: require DISTANCE_SENSOR **stream alive** (scan-height ok). Post-descend: require trusted AGL in `[HOVER_MIN, HOVER_MAX]` before pulse |
