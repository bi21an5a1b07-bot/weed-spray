...
**Hover command (app seam, PR #25):** default `WEED_HOVER_ALTITUDE_MODE=ned` still does `goto_ned(..., down=-hover_agl_m)` (SIH). Opt-in `offboard_agl` **requires** `WEED_LIDAR_EXPECTED=1` (Gazebo belly lidar). SIH cannot fake this — leave it unset / `false`. Sequence: latch scan-height `distance_sensor_stream_alive` **before** NED hover (ds and `relative_alt` both in `[1, 5]` m; **flat ds≈rel unlocks** — that is Gazebo, not SIH-only). `relative_alt is None` or lag (ds scan-height while rel still ~hover) do **not** unlock. Then NED approach. After descend: trusted `distance_reading_m` in `[hover_min_m, hover_max_m]` **and** the latched stream_ok. Only then `Vehicle.goto_global_agl` — MAVSDK `AltitudeType.AGL` / `MAV_FRAME_GLOBAL_TERRAIN_ALT_INT`. `hover_agl_m` is **positive AGL metres**. **No PX4 params written.**

**Not claimed live:** PX4 `MPC_ALT_MODE` terrain hold is still **Position/Altitude only, not Offboard** ([PX4 terrain following / holding](https://docs.px4.io/main/en/flying/terrain_following_holding.html)). Do **not** fake AGL with local `z`. Do not invent `EKF2_RNG_*`, `COM_RCL_EXCEPT`, `NAV_RCL_ACT=0`, or `COM_RC_IN_MODE=4`. Whether terrain-alt Offboard actually holds 0.15–0.30 m on `gz_x500_lidar_down` is **operator WSL UAT** — put `WEED_HOVER_ALTITUDE_MODE=offboard_agl` and `WEED_LIDAR_EXPECTED=1` on the **backend** (`uv run weed-spray`), not on `make sitl-gz`. Grade step 7 from `DISTANCE_SENSOR`, not `relative_alt`. Paste `var/last-run.md` if it does not hold.
