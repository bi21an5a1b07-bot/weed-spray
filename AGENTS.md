...
- After Python edits: `make check` (`ruff check` + `ruff format --check` + pytest). Config: `[tool.ruff]` in `pyproject.toml`. **TDD:** failing test first, then code; see `GROK.md`. Live accept is not a unit test.
