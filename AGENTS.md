...
- After Python edits: `make check` (`ruff check` + `ruff format --check` + pytest). Config: `[tool.ruff]` in `pyproject.toml`. **TDD:** failing test first, then code; see `GROK.md`. Live accept is not a unit test. New/changed functions get Google-style docstrings; public API also in `docs/code-reference.md`.
- Host tools: `mise.toml` (Python 3.11, uv, Node 26, github-cli 2). Packages stay in uv/npm.
