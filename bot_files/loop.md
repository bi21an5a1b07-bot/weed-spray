...
|---|---|---|---|
| **14540** | UDP | Python backend (MAVSDK / offboard API) | PX4 offboard. [PX4 remote API port](https://docs.px4.io/main/en/simulation/). |
| 14550 | UDP | optional QGC **inside WSL** | PX4 GCS link. Do not hairpin to Windows QGC in v1. |
| 8554 | TCP | MediaMTX | `rtsp://127.0.0.1:8554/cam` from the looped file. Backend + YOLO read this. |
| 8080 | TCP | dashboard | Browser on localhost. Grok Build may change the port; document it here if they do. |
| 8090 | TCP | YOLO worker | Backend pulls boxes or worker pushes them. Localhost only. |
...
| Item | Owner |
|---|---|
| Compose file (PX4 SIH + mock RTSP only) | Grok Build |
| Lawn `.mp4` in the repo | operator / Grok Build |
| Offboard + geofence + confirm API on 14540 | Grok Build (notes in `px4/offboard.md`) |
| Hardware lidar UART (TFmini I2C is a dead end) | `@parts` / `@hardware` — not this loop |
