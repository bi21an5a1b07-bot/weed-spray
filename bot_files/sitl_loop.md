...
Prefer the lightest PX4 SITL that can take Offboard. On ~25 GB RAM, Gazebo + YOLO is tight. Default PX4 is **SIH** (`px4io/px4-sitl`, `PX4_SIM_MODEL=sihsim_quadx`). SIH has IMU/GPS/baro/mag, not lidar or optical flow ([comparison](https://docs.px4.io/main/en/simulation/#simulator-comparison)). Hover AGL in this loop is therefore a **backend-recorded** number (commanded **0.27 m**, band 0.24–0.32 m: S500/gz x500 gear ~0.22 m + ~2 in). `sitl/summaries/` will mark rangefinder `missing` unless Grok Build later adds Gazebo `gz_x500_lidar_down` ([rangefinders](https://docs.px4.io/main/en/sensor/rangefinders)).
...
A weekday 07:00 America/Denver routine exists and is **paused** until this script has one manual green pass **and** a compose file is in the repo.

## Compose contract (Grok Build WSL repo)

This cloud VM **cannot** read `/home/behmann/src/grok/drone_control`. Compose contents here: **unknown** (owner: Grok Build). Do not start Docker from here.

Locked ports / image for acceptance. If the repo differs, that is a mismatch.

| Piece | Contract |
|---|---|
| PX4 image | `px4io/px4-sitl`, `PX4_SIM_MODEL=sihsim_quadx` ([pre-built SITL](https://docs.px4.io/main/en/simulation/px4_sitl_prebuilt_packages)) |
| RTSP | `rtsp://127.0.0.1:8554/cam` (file, not Gazebo cam) |
| backend HTTP | `8000` |
| dashboard | `8080` |
| injector | `8090` |
| MAVLink offboard | UDP **14540** |

Mismatch vs this file before 2026-08-30 evening: backend HTTP **8000** was not listed (only UDP 14540). Dashboard 8080 and injector 8090 already matched. WSL compose itself: unknown until the operator pastes it.

## Open

| Item | Owner |
|---|---|
| Confirm WSL compose matches the table above | operator / Grok Build |
| Lawn `.mp4` in the repo | operator / Grok Build |
