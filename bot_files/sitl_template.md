...
Spray-hover samples only (`phase == spray_hover`). `t`, `agl_m`, `detection_id`. Source: `DISTANCE_SENSOR` / `distance_sensor.current_distance` (m), downward. **If no distance sensor: `missing` (SIH).** Target **0.24–0.32 m** (commanded 0.27 m: S500/gz x500 gear ~0.22 m + ~2 in).
